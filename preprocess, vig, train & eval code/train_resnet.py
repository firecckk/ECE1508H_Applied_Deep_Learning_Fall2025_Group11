...
example usage: python train_resnet.py --data-dir C:/Users/hardy/Desktop/OCT --pin-memory --mixed-precision
'''
import os
import time
import json
...
# build a torchvision resnet
def build_resnet(model_name: str, num_classes: int = 4, in_channels: int = 1, pretrained: bool = False):
...
def run_experiment(cfg, hp, out_dir, device, verbose=True):
    ...
    # build resnet model
    model, pretrained_weights_name = build_resnet(
        cfg['model_name'],
    ...
    # apply kaiming initialization if not pretrained
    ...
    for epoch in range(1, epochs + 1):
        ...
        # scheduler step on val_f1 (maximize)
        try:
        ...
        # early stopping (maximize val_f1)
...
def main(args):
    ...
    print("building initial dataloaders (informational run) - this runs preprocessing now...")
    ...
    # hyperparameter candidate parsing
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='train resnet on oct dataset (with hyperparam search)')
    parser.add_argument('--data-dir', type=str, required=True, help='dataset path')
    parser.add_argument('--model', type=str, default='resnet50', help='resnet variant name from torchvision.models (default resnet50)')
    parser.add_argument('--pretrained', action='store_true', help='use imagenet pretrained weights')
    parser.add_argument('--rgb', action='store_true', help='use rgb images (default is grayscale)')
    parser.add_argument('--img-size', type=int, default=224, help='image size (square). adjust if using special pretrained models)')
    ...
    parser.add_argument('--save-dir', type=str, default='./outputs/resnet_search')
