'''
train a convnext classifier
example usage: python train_convnext.py --data-dir C:/Users/hardy/Desktop/OCT --pin-memory --mixed-precision
'''
import os
import time
import json
...
def build_convnext(
    ...
    # adjust internal expected image size
    ...
    # replace first conv2d that has in_channels==3
    if in_channels != 3:
        replaced = False
    ...
    # replace last linear inside a module
    def replace_last_linear_in_module(module: nn.Module, num_classes: int) -> bool:
        # flatten immediate children with their names to allow assignment
        children = list(module.named_children()) # list of (name, module)
        ...
        for name, child in reversed(children):
            if isinstance(child, nn.Linear):
                ...
                try:
                    # if module is Sequential and name is numeric index
                    if name.isdigit():
                        module[int(name)] = new_lin
                    else:
            ...
            # if child is a Sequential or Module, attempt to find linear inside it recursively
            if isinstance(child, (nn.Sequential, nn.Module)):
                replaced = replace_last_linear_in_module(child, num_classes)
    ...
    for attr_name in ['classifier', 'head', 'fc', 'classifier[0]', 'head.fc']:
        if replaced_head:
            break
        ...
        if hasattr(model, attr_name):
            ...
            # if it's a linear directly
            if isinstance(attr, nn.Linear):
                in_features = attr.in_features
                setattr(model, attr_name, nn.Linear(in_features, num_classes))
                replaced_head = True

                break

            # if it's a sequential or module container, try to replace its last linear
            if isinstance(attr, (nn.Sequential, nn.Module)):
                if replace_last_linear_in_module(attr, num_classes):
                    setattr(model, attr_name, attr)
                    replaced_head = True

                    break

    # if still not replaced, try scanning named_children for a linear to replace
    if not replaced_head:
        for name, child in reversed(list(model.named_children())):
            if isinstance(child, nn.Linear):
                in_features = child.in_features
                new_lin = nn.Linear(in_features, num_classes)
            ...
            if isinstance(child, (nn.Sequential, nn.Module)):
                if replace_last_linear_in_module(child, num_classes):
                    setattr(model, name, child)
                    replaced_head = True

                    break

    # final fallback: attach attribute 'head' as a linear
    if not replaced_head:
...
# train for one epoch and return avg loss and metrics
...
# evaluate over loader and return avg loss and metrics
...
# run a single experiment (one hyperparameter configuration)
def run_experiment(cfg, hp, out_dir, device, verbose=True):
    ...
    # build dataloaders for this run
    ...
    t0 = time.time()
    out = build_dataloaders(dataloader_cfg)  # preprocess.py handles detailed output
    t1 = time.time()

    print(f"dataloaders ready (took {t1 - t0:.1f}s)")
    ...
    # build model
    model, pretrained_weights_name = build_convnext(
        cfg['model_name'],
    ...
    # apply kaiming init if not pretrained
    ...
    for epoch in range(1, epochs + 1):
        ...
        # scheduler step with val_f1 (we prefer higher f1)
        try:
        ...
        # check early stopping (maximize val_f1)
        improved = val_f1 > best_val_f1 + 1e-8 # prevents treating floating-point noise as real progress
    ...
    # restore best and run final test
    ...
    # plot train vs val and horizontal test line
...
def main(args):
    ...
    # informational dataloader build
    ...
    print(f"initial preprocessing done (took {t1 - t0:.1f}s)\n")

    # print dataset sizes and a sample of computed stats
    ...
    # candidate lists
    lr_list = [float(x) for x in args.lr_list.split(',') if x.strip()]
    wd_list = [float(x) for x in args.wd_list.split(',') if x.strip()]
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='train convnext on oct dataset (with hp search)')
    parser.add_argument('--data-dir', type=str, required=True, help='dataset path')
    parser.add_argument('--convnext', type=str, default='convnext_tiny', help='model name (default convnext_tiny)')
    ...
    parser.add_argument('--img-size', type=int, default=224, help='image size (use 384 for some pretrained variants)')
    parser.add_argument('--epochs', type=int, default=50, help='number of epochs (default 50)')
    parser.add_argument('--batch-size', type=int, default=32, help='default batch size (informational)')
    parser.add_argument('--lr', type=float, default=1e-4, help='default lr (not used when searching)')
    ...
    parser.add_argument('--num-workers', type=int, default=max(0, min(12, (os.cpu_count() or 4) - 1)), help='dataloader workers')
    parser.add_argument('--seed', type=int, default=666)
    parser.add_argument('--save-dir', type=str, default='./outputs/convnext_search')
    ...
    parser.add_argument('--num-trials', type=int, default=5, help='number of random trials (for random search)')
    parser.add_argument('--early-stopping-patience', type=int, default=10, help='early stopping patience (val f1)')
