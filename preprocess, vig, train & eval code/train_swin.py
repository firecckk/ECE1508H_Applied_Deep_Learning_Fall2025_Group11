'''
train a swin transformer classifier
- example usage: python train_swin.py --data-dir C:/Users/hardy/Desktop/OCT --pretrained --pin-memory --mixed-precision
'''
import os
import time
import json
...
# xavier initialization if not pretrained
...
# find first conv2d with in_channels = 3
def find_first_conv3(model: nn.Module):
...
def replace_first_conv(model: nn.Module, in_channels: int, pretrained: bool):
    parent, attr, conv = find_first_conv3(model)
...
# swin transformer
def build_swin(model_name: str, num_classes: int = 4, in_channels: int = 1, pretrained: bool = False, img_size: int = 224):
    ...
    if in_channels != 3:
        replaced = replace_first_conv(model, in_channels=in_channels, pretrained=pretrained)
        ...
        if not replaced:
            ...
            if not tried:
                ...
                if conv is not None:
                    replace_first_conv(model, in_channels=in_channels, pretrained=pretrained)
    ...
    if hasattr(model, 'head') and isinstance(getattr(model, 'head'), nn.Linear):
        ...
        replaced_head = True
    elif hasattr(model, 'classifier') and isinstance(getattr(model, 'classifier'), nn.Linear):
        in_features = model.classifier.in_features
        model.classifier = nn.Linear(in_features, num_classes)
        replaced_head = True
    else:
        if hasattr(model, 'heads') and isinstance(getattr(model, 'heads'), nn.Module):
            # replace last linear inside heads
            for nm, m in reversed(list(model.heads.named_modules())):
                if isinstance(m, nn.Linear):
                    ...
                    setattr(parent, last, nn.Linear(m.in_features, num_classes))
                    replaced_head = True
...
# run one hyperparameter config
def run_experiment(cfg, hp, out_dir: Path, device, verbose=True):
    ...
    # prepare dataloader cfg for this batch size
    ...
    print(f"dataloaders built in {build_t1 - build_t0:.1f}s")
    ...
    # build the model
    model, pretrained_weights_name = build_swin(cfg['model_name'], num_classes=num_classes, in_channels=in_channels, pretrained=cfg['pretrained'], img_size=cfg['img_size'])
    device_str = str(device)
    model = model.to(device)

    # apply xavier if not pretrained
    ...
    for epoch in range(1, epochs + 1):
        epoch_t0 = time.time()
        train_loss, train_acc, train_prec, train_rec, train_f1 = train_one_epoch(model, train_loader, criterion, optimizer, device, scaler, cfg['mixed_precision'])
        val_loss, val_acc, val_prec, val_rec, val_f1 = evaluate(model, val_loader, criterion, device)

        # scheduler (using validation f1)
        try:
        ...
        # print epoch summary
        print(f"epoch {epoch:03d}/{epochs} | time: {epoch_time:.1f}s | train loss: {train_loss:.4f} acc: {train_acc*100:.2f}% f1: {train_f1:.4f} | val loss: {val_loss:.4f} acc: {val_acc*100:.2f}% f1: {val_f1:.4f}")

        # save last checkpoint for the run
        out_dir.mkdir(parents=True, exist_ok=True)
        torch.save({'epoch': epoch, 'model_state': model.state_dict(), 'optimizer_state': optimizer.state_dict(), 'val_f1': val_f1, 'val_loss': val_loss}, out_dir / 'last_ckpt.pth')
    ...
    # restore best state for final test
    ...
    with csv_path.open('w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        header = ['epoch','train_loss','train_acc','train_prec','train_rec','train_f1','val_loss','val_acc','val_prec','val_rec','val_f1']
    ...
    # plot metrics vs epochs (train + val curves, test shown as dashed horizontal line)
...
def main(args):
    ...
    # build once as informational run to display dataset sizes and stats
    ...
    print(f"dataloaders (informational) built in {t1 - t0:.1f}s")
    ...
    for i, hp in enumerate(combos, start=1):
        ...
        # run experiment and collect summary
        summary = run_experiment(cfg, hp, run_out, device, verbose=True)
        all_summaries.append(summary)

        # update global best by validation f1
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train Swin Transformer on OCT dataset with hyperparameter search')
    parser.add_argument('--data-dir', type=str, required=True, help='dataset path')
    parser.add_argument('--model', type=str, default='swin_v2_t', help='swin variant name from torchvision.models (default: swin_v2_t)')
    ...
    parser.add_argument('--batch-size', type=int, default=32, help='default batch size shown in informational run')
    ...
    parser.add_argument('--save-dir', type=str, default='./outputs/swin_search')
